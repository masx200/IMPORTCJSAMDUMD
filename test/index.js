...
Object.assign(packagealias, {
    react: 'https://cdn.staticfile.org/react/16.10.2/umd/react.production.min.js',
    vue: 'https://cdn.staticfile.org/vue/2.6.10/vue.esm.browser.min.js',
    jquery: 'https://cdn.staticfile.org/jquery/3.4.1/jquery.min.js',
    bootstrap: 'https://cdn.staticfile.org/twitter-bootstrap/4.3.1/js/bootstrap.bundle.min.js',
    hljs: 'https://cdn.jsdelivr.net/npm/highlight.js@9.15.10/lib/highlight.min.js',
    md5: 'https://cdn.jsdelivr.net/gh/masx200/masx200.github.io/src/assetsjs/md5.min.js'
});
const allpro = Promise.all([
    importcjsamdumd('https://cdn.jsdelivr.net/npm/axios@0.19.2/dist/axios.js').then(logger),
    importcjsamdumd('./circular.js').catch(console.error),
    importcjsamdumd('./cjs3.js', 'https://cdn.jsdelivr.net/npm/@masx200/event-emitter-target@1.1.5/dist/index.min.js').then(logger),
    importcjsamdumd([
        './array.json',
    ...
    importcjsamdumd('https://cdn.staticfile.org/react/16.10.2/umd/react.production.min.js', 'react').then(logger).then(() => {
        logger('react', requirepackage('react'));
    }),
    importcjsamdumd('https://cdn.jsdelivr.net/npm/fast-xml-parser@3.14.0/src/parser.min.js').then(logger),
    importcjsamdumd([
    ...
    importcjsamdumd([
        ...
        logger('jquery', requirepackage('jquery')), logger('bootstrap', requirepackage('bootstrap'));
    }),
    importcjsamdumd('https://cdn.jsdelivr.net/npm/highlight.js@9.15.10/lib/highlight.min.js', 'hljs').then(logger).then(() => {
        logger('hljs', requirepackage('hljs'));
    }),
    importcjsamdumd('https://cdn.jsdelivr.net/gh/masx200/masx200.github.io/src/assetsjs/md5.min.js', 'md5').then(logger).then(() => {
    ...
    importcjsamdumd('https://cdn.jsdelivr.net/npm/core-js@3.6.4/modules/es.map.min.js').then(logger)
]);
logger(Object.assign({}, cjsamdumd));
allpro.then(() => {
    const allids = getmoduleids();
    logger(allids);
    logger(allids.map(id => {
        return [
            id,
            getmodulesource(id)
        ];
